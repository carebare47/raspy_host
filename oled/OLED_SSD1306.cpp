...
inline boolean OLED_SSD1306::isSPI(void) {
	return (cs != -1 ? true : false);
}
inline boolean OLED_SSD1306::isI2C(void) {
	return (cs == -1 ? true : false);
}
// Low level I2C and SPI Write function
inline void OLED_SSD1306::fastSPIwrite(uint8_t d) {
	bcm2835_spi_transfer(d);
}
inline void OLED_SSD1306::fastI2Cwrite(uint8_t d) {
	bcm2835_spi_transfer(d);
}
inline void OLED_SSD1306::fastSPIwrite(char* tbuf, uint32_t len) {
	bcm2835_spi_writenb(tbuf, len);
}
inline void OLED_SSD1306::fastI2Cwrite(char* tbuf, uint32_t len) {
...
boolean OLED_SSD1306::oled_is_spi_proto(uint8_t OLED_TYPE) 
{
	switch (OLED_TYPE)
	{
		case OLED_OLED_SPI_128x64:
			return true;
		break;
	}
...
boolean OLED_SSD1306::select_oled(uint8_t OLED_TYPE) 
{
	// Default type
...
//
boolean OLED_SSD1306::init(int8_t DC, int8_t RST, int8_t CS, uint8_t OLED_TYPE) 
{
  rst = RST;	// Reset Pin
...
boolean OLED_SSD1306::init(int8_t RST, uint8_t OLED_TYPE) 
{
...
void OLED_SSD1306::ssd1306_command(uint8_t c0, uint8_t c1) 
{ 
	char buff[3] ;
	buff[1] = c0;
	buff[2] = c1;
...
void OLED_SSD1306::ssd1306_command(uint8_t c0, uint8_t c1, uint8_t c2) 
{ 
	char buff[4] ;
...
void OLED_SSD1306::ssd1306_data(uint8_t c) 
{
	// SPI
  if ( isSPI())
  {
